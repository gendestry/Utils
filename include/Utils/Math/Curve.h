...
class Curve
{
  ...
  public:
    ...
    void setPeaks(const uint16_t peaks)
    {
        m_peaks = peaks;
        fill();
    }
...
class Sinusoid : public Curve
{
  ...
  public:
    Sinusoid(uint16_t length, uint16_t peaks = 1U) : Curve(SINUSOID, length, peaks) { init(); }
...
class SinusoidHalf : public Curve
{
  ...
  public:
    SinusoidHalf(uint16_t length, uint16_t peaks = 1U) : Curve(SINUSOID, length, peaks) { init(); }
...
class Triangle : public Curve
{
  ...
  public:
    Triangle(uint16_t length, uint16_t peaks = 1U) : Curve(TRIANGLE, length, peaks) { init(); }
...
class Square : public Curve
{
  ...
  public:
    Square(uint16_t length, uint16_t peaks = 1U) : Curve(SQUARE, length, peaks) { init(); }
...
class Line : public Curve
{
  ...
  public:
    Line(uint16_t length, uint16_t peaks = 1U) : Curve(LINE, length, peaks) { init(); }
